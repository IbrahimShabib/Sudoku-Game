...
def makeConsistant(domains, xi, xj):
    pruned = False
    # For Sudoku, xi and xj are inconsistent ONLY if xj has only one possible value
    # and that value is the same as one in xi's domain.
    if len(domains[xj]) == 1:
        fixed_val = domains[xj][0]
        if fixed_val in domains[xi]:
            domains[xi].remove(fixed_val)
            pruned = True
    return pruned
